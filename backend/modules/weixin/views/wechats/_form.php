...
<div class="col-sm-12">
    ...
    <?= $form->field($model, 'accountlink')->hint('您的微信帐号或是易信帐号，引导关注页面，直接点击到引导关注页面')?>
    <?= $form->field($model, 'original')->label('原始帐号')->hint('微信公众帐号的原ID串，'.HTML::a('怎么查看微信的原始帐号？',['index','act'=>'help'], ['target'=>'blank']))?>
    <?php if(empty($model->weid)){?>
    <?= $form->field($model, 'qrcode')->widget(FileInput::classname(), [
    ...
    'options' => ['accept' => 'image/*','multiple' => true],'pluginOptions' => [
        'initialPreview'=>[
            Html::img(yii::$app->urlManager->createAbsoluteUrl("upload/weixin/qrcode_{$model->weid}.jpg"), ['class'=>'file-preview-image', 'alt'=>'The Moon', 'title'=>'The Moon']),
                ],
        'uploadExtraData' => [
            'album_id' => 20,
            'cat_id' => 'Nature'
    ...
    'options' => ['accept' => 'image/*','multiple' => true],'pluginOptions' => [
        'initialPreview'=>[
            Html::img("upload/weixin/headimg_{$model->weid}.jpg", ['class'=>'file-preview-image', 'alt'=>'The Moon', 'title'=>'The Moon']),
                ],
        'uploadExtraData' => [
            'album_id' => 20,
            'cat_id' => 'Nature'
