...
class Communication {
	 ...
	 public static function ihttp_request($url, $post = '', $extra = array(), $timeout = 60) {
		...
		if(function_exists('curl_init') && function_exists('curl_exec')) {
			...
			if($errno || empty($data)) {
				...
				return static::ihttp_response_parse($data);
			}
		}
		...
		if (!$fp) {
			...
			fclose($fp);
			return static::ihttp_response_parse($content, true);
		}
	}
	...
	public static function ihttp_response_parse($data, $chunked = false) {
		...
		$ischunk = false;
		if($chunked && $ischunk) {
			$rlt['content'] = static::ihttp_response_parse_unchunk($split1[1]);
		} else {
		...
		$rlt['meta'] = $data;
		if($rlt['code'] == '100') {
			return static::ihttp_response_parse($rlt['content']);
		}
		return $rlt;
	}
	...
	public static function ihttp_get($url) {
		return static::ihttp_request($url);
	}
	
	 public static function ihttp_post($url, $data) {
		$headers = array('Content-Type' => 'application/x-www-form-urlencoded');
		return static::ihttp_request($url, $data, $headers);
	}
	
	public static function ihttp_email($to, $subject, $body) {
		static $mailer;
		set_time_limit(0);
